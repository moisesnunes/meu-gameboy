...
static void set_style(void)
{
    ...
    s.WindowRounding   = 6.0f; s.WindowBorderSize = 1.0f;
    s.ChildRounding    = 4.0f; s.PopupRounding    = 6.0f;
    s.FramePadding     = ImVec2(8.0f, 4.0f); s.FrameRounding = 4.0f;
    s.ItemSpacing      = ImVec2(8.0f, 6.0f);
    s.ScrollbarSize    = 12.0f; s.ScrollbarRounding = 4.0f;
...
static void push_recent(const char *path)
{
    ...
    for (auto it = s_recent_roms.begin(); it != s_recent_roms.end(); ++it)
        if (*it == p) { s_recent_roms.erase(it); break; }
...
{
    if (!gba->cart.rom) return "Idle";
    if (!gba->debug.enabled) return "Rodando";
    switch (gba->debug.state) {
    case GBA_DEBUG_PAUSED:   return "Pausado";
    case GBA_DEBUG_STEPPING: return "Step";
    case GBA_DEBUG_RUNNING:  return "Rodando";
...
static void draw_status_bar(struct gba *gba)
{
    ...
    if (!ImGui::Begin("##gba_status_bar", nullptr, flags)) { ImGui::End(); return; }
    ...
    ImGui::SameLine(); ImGui::TextColored(color_gray, "|"); ImGui::SameLine();
    ...
    if (avail > rw) ImGui::SameLine(ImGui::GetCursorPosX() + avail - rw);
...
static void draw_game_output(void)
{
    ...
    ImGui::Begin("Sa" "\xc3\xad" "da##gba_screen", &s_show_screen,
    ...
    if (off_x < ImGui::GetCursorPosX()) off_x = ImGui::GetCursorPosX();
    if (off_y < ImGui::GetCursorPosY()) off_y = ImGui::GetCursorPosY();
    ...
    if (s_fast_forward) {
        ...
        char ff[16]; snprintf(ff, sizeof(ff), ">> %.0fx", s_ff_speed_factor);
...
static void draw_main_menu(struct gba *gba)
{
    ...
    if (ImGui::BeginMenu("Emulador")) {
        if (ImGui::MenuItem("Pausar", "F5", paused, has_rom)) {
            if (paused) gba->debug.state = GBA_DEBUG_RUNNING;
            else { gba->debug.enabled = true; gba->debug.state = GBA_DEBUG_PAUSED; }
        }
        ...
        if (ImGui::BeginMenu("Velocidade do FF", has_rom)) {
            ...
            ImGui::Separator();
            if (ImGui::MenuItem("2x",  nullptr, s_ff_speed_factor == 2.0f)) s_ff_speed_factor = 2.0f;
            if (ImGui::MenuItem("4x",  nullptr, s_ff_speed_factor == 4.0f)) s_ff_speed_factor = 4.0f;
            if (ImGui::MenuItem("8x",  nullptr, s_ff_speed_factor == 8.0f)) s_ff_speed_factor = 8.0f;
            ImGui::EndMenu();
        }
    ...
    /* Vídeo */
    if (ImGui::BeginMenu("V" "\xc3\xad" "deo")) {
        ...
        if (ImGui::BeginMenu("Escala")) {
            if (ImGui::MenuItem("Ajustar \xc3\xa0 janela",      nullptr, s_video_scale == VIDEO_SCALE_FIT))        s_video_scale = VIDEO_SCALE_FIT;
            if (ImGui::MenuItem("Inteira autom\xc3\xa1tica",    nullptr, s_video_scale == VIDEO_SCALE_INTEGER))    s_video_scale = VIDEO_SCALE_INTEGER;
            if (ImGui::MenuItem("Ajustar \xc3\xa0 largura",     nullptr, s_video_scale == VIDEO_SCALE_FIT_WIDTH))  s_video_scale = VIDEO_SCALE_FIT_WIDTH;
            if (ImGui::MenuItem("Ajustar \xc3\xa0 altura",      nullptr, s_video_scale == VIDEO_SCALE_FIT_HEIGHT)) s_video_scale = VIDEO_SCALE_FIT_HEIGHT;
            ImGui::Separator();
            for (int sc = 1; sc <= 6; ++sc) {
                char lbl[48];
                snprintf(lbl, sizeof(lbl), "%d\xc3\x97  (%d \xc3\x97 %d)", sc, GBA_LCD_W*sc, GBA_LCD_H*sc);
                if (ImGui::MenuItem(lbl, nullptr, s_video_scale == sc)) s_video_scale = sc;
            }
            ImGui::EndMenu();
        }
        ...
        if (ImGui::MenuItem("Mostrar FPS", nullptr, s_show_fps)) s_show_fps = !s_show_fps;
        ...
        if (ImGui::BeginMenu("Cor de Fundo")) {
            ...
            ImGui::SameLine(); ImGui::Text("Cor de fundo");
            ImGui::Separator();
            if (ImGui::MenuItem("Preto"))      { s_bg_color[0]=0.0f; s_bg_color[1]=0.0f; s_bg_color[2]=0.0f; }
            if (ImGui::MenuItem("Cinza (padr\xc3\xa3o)")) { s_bg_color[0]=0.1f; s_bg_color[1]=0.1f; s_bg_color[2]=0.1f; }
            if (ImGui::MenuItem("Azul marinho")) { s_bg_color[0]=0.05f; s_bg_color[1]=0.05f; s_bg_color[2]=0.15f; }
            ImGui::EndMenu();
        }
    ...
    if (ImGui::BeginMenu("Debug")) {
        if (ImGui::MenuItem("Ativar Debug", nullptr, &gba->debug.enabled)) {
            ...
            else                    gba->debug.state = GBA_DEBUG_RUNNING;
        }

        ImGui::Separator();
        ImGui::MenuItem("Sa" "\xc3\xad" "da do Jogo", nullptr, &s_show_screen);
        ...
        ImGui::Separator();
        if (ImGui::BeginMenu("V" "\xc3\xad" "deo", dbg)) {
        ...
        if (ImGui::BeginMenu("Tamanho de Fonte")) {
            if (ImGui::MenuItem("Pequena", nullptr, s_debug_font_size==0)) { s_debug_font_size=0; apply_font_scale(); }
            if (ImGui::MenuItem("Normal",  nullptr, s_debug_font_size==1)) { s_debug_font_size=1; apply_font_scale(); }
            if (ImGui::MenuItem("Grande",  nullptr, s_debug_font_size==2)) { s_debug_font_size=2; apply_font_scale(); }
            if (ImGui::MenuItem("Enorme",  nullptr, s_debug_font_size==3)) { s_debug_font_size=3; apply_font_scale(); }
            ImGui::EndMenu();
        }
    ...
    if (ImGui::BeginMenu("Sobre")) {
        if (ImGui::MenuItem("Sobre meu-GBA...")) s_show_about = true;
        ImGui::EndMenu();
    }

    /* FPS lado direito */
    if (s_show_fps) {
        char fps[32]; snprintf(fps, sizeof(fps), "%.0f FPS", s_fps_current);
        float w = ImGui::GetContentRegionAvail().x;
        float tw = ImGui::CalcTextSize(fps).x + 8.0f;
        if (w > tw) ImGui::SameLine(ImGui::GetCursorPosX() + w - tw);
    ...
    /* Popup Sobre */
    if (s_show_about) { ImGui::OpenPopup("Sobre##gba_modal"); s_show_about = false; }
...
bool gba_debug_ui_init(struct gba *gba, SDL_Window *window)
{
    ...
    if (roboto) io.FontDefault = roboto;
    apply_font_scale();
...
void gba_debug_ui_process_event(struct gba *gba, SDL_Event *event)
{
    ...
    if (event->type == SDL_EVENT_KEY_DOWN && !event->key.repeat) {
        ...
        if (key == SDLK_F5) {
            if (gba->debug.state == GBA_DEBUG_RUNNING) gba->debug.state = GBA_DEBUG_PAUSED;
            else                                        gba->debug.state = GBA_DEBUG_RUNNING;
        }
...
{
    if (!s_window || !s_gl_context) return;

    /* FPS */
    {
        uint64_t now = SDL_GetTicksNS();
        s_fps_frames++;
        if (!s_fps_last_ns) s_fps_last_ns = now;
...
void gba_debug_ui_destroy(void)
{
    ...
    SDL_GL_MakeCurrent(s_window, s_gl_context);
    if (s_game_tex) { glDeleteTextures(1, &s_game_tex); s_game_tex = 0; }
