/*
 * gba_apu.c — GBA Audio Processing Unit.
 *
 * Emulates four legacy GB-compatible channels (square×2, wave, noise) plus
 * two DMA FIFO PCM channels (A and B).  Samples are mixed at GBA_APU_SAMPLE_RATE
 * and written to a double-buffer consumed by the SDL audio callback.
 */
...
void gba_apu_sync(struct gba *gba)
{
     ...
     while (apu->next_sample_cycles <= 0)
     {
          ...
          if (apu->ch1.enabled)
               ch_square_advance(&apu->ch1.div, &apu->ch1.duty_pos, cyc);
          if (apu->ch2.enabled)
               ch_square_advance(&apu->ch2.div, &apu->ch2.duty_pos, cyc);
          if (apu->ch3.enabled)
               ch3_advance(&apu->ch3, cyc);
          if (apu->ch4.enabled)
               ch4_advance(&apu->ch4, cyc);
          ...
          if (master_on && apu->ch1.enabled && apu->ch1.dac_on && !apu->mute_ch1)
               s1 = duty_table[apu->ch1.duty][apu->ch1.duty_pos]
                        ? (int16_t)(apu->ch1.env.volume * 512)
                        : 0;
          ...
          if (master_on && apu->ch2.enabled && apu->ch2.dac_on && !apu->mute_ch2)
               s2 = duty_table[apu->ch2.duty][apu->ch2.duty_pos]
                        ? (int16_t)(apu->ch2.env.volume * 512)
                        : 0;
          ...
          int16_t ol = (int16_t)(out_l > 32767 ? 32767 : out_l < -32768 ? -32768
                                                                        : out_l);
          int16_t or_ = (int16_t)(out_r > 32767 ? 32767 : out_r < -32768 ? -32768
                                                                         : out_r);
