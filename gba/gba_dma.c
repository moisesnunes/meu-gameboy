/*
 * gba_dma.c — GBA DMA controller (4 channels: DMA0–DMA3).
 *
 * Handles address/count latching, timing modes (immediate/VBlank/HBlank/special),
 * audio FIFO replenishment, and ROM-to-RAM wait-state accounting.
 */
...
static void dma_advance_sampling_time(struct gba *gba, int cycles, int active_dma)
{
     ...
     if (gba->timestamp >= gba->sync.next_event[GBA_SYNC_DMA])
     {
          bool ran_dma = false;
          ...
          if (gba->timestamp < gba->sync.next_event[GBA_SYNC_DMA] + 6)
               return;
...
void gba_dma_write_ctrl(struct gba *gba, int n, uint16_t val)
{
     ...
     if (!was_enabled && ch->enable)
     {
          ...
          ch->src = ch->src_latch;
          ch->dst = ch->dst_latch;
          ch->count = ch->count_latch ? ch->count_latch : ((n == 3) ? 0x10000 : 0x4000);
          ch->video_oneshot = (n == 3 &&
                               ch->timing == GBA_DMA_SPECIAL &&
                               !ch->repeat &&
                               ch->count == 1);
...
static int dma_addr_step(enum gba_dma_addr_mode mode, bool word_32)
{
     ...
     switch (mode)
     {
     case GBA_DMA_ADDR_INC:
          return unit;
     case GBA_DMA_ADDR_DEC:
          return -unit;
     case GBA_DMA_ADDR_FIXED:
          return 0;
     case GBA_DMA_ADDR_RELOAD:
          return unit;
     default:
          return unit;
     }
...
static void dma_run(struct gba *gba, int n)
{
     ...
     bool timed_if_sampling = !ch->word_32 &&
                              (ch->src & ~1U) == REG_IF &&
                              original_count > 32;
     bool timed_vcount_sampling = !ch->word_32 &&
                                  ch->src_mode == GBA_DMA_ADDR_FIXED &&
                                  (ch->src & ~1U) == REG_VCOUNT &&
                                  original_count > 32;
               ...
               else if (timed_if_sampling)
               {
                    ...
                    if (gba->gpu.hblank_irq_en)
                    {
                         int32_t until_hblank =
                             gba->sync.next_event[GBA_SYNC_GPU] - gba->timestamp;
                         if (ch->timing == GBA_DMA_NOW)
                         {
                              if (!gba->gpu.hblank && until_hblank == 0)
                                   v |= (uint16_t)(1U << GBA_IRQ_HBLANK);
                              else if (!gba->gpu.hblank)
                                   v &= ~(uint16_t)(1U << GBA_IRQ_HBLANK);
                         }
                         else
                         {
                              int32_t hblank_start =
                                  gba->sync.next_event[GBA_SYNC_GPU] - 224;
                              if (gba->gpu.hblank &&
                                  gba->timestamp >= hblank_start + 4)
                                   v |= (uint16_t)(1U << GBA_IRQ_HBLANK);
                              else
                                   v &= ~(uint16_t)(1U << GBA_IRQ_HBLANK);
                         }
               ...
               else
               {
                    ...
                    if (timed_io_sampling || timed_vcount_sampling)
                    {
                         int32_t status_hold_cycles =
                             (ch->timing == GBA_DMA_HBLANK) ? 4 : 2;
                         int32_t since_line_start =
                             gba->timestamp - gba->sync.last_sync[GBA_SYNC_GPU];

                         if (!gba->gpu.hblank && since_line_start >= 0 &&
                             since_line_start <= status_hold_cycles)
                         {
                              uint8_t prev_vcount = gba->gpu.vcount ? (uint8_t)(gba->gpu.vcount - 1) : (uint8_t)(GBA_LCD_TOTAL_LINES - 1);

                              if (timed_io_sampling)
                              {
                                   v |= 2;
                                   if (prev_vcount == gba->gpu.vcount_trigger)
                                        v |= 4;
                                   else
                                        v &= (uint16_t)~4U;
                              }
                             ...
                             gba->gpu.hblank)
                         {
                              int32_t hblank_start =
                                  gba->sync.next_event[GBA_SYNC_GPU] - 224;
                              if (gba->timestamp < hblank_start + 2)
                                   v &= (uint16_t)~2U;
                         }
                    }
                    if (src & 2U)
...
void gba_dma_sync(struct gba *gba)
{
     ...
     bool precise_hblank_event = false;
     bool precise_video_oneshot = false;
...
static void dma_trigger_timing(struct gba *gba, enum gba_dma_timing timing)
{
     ...
     if (triggered)
          gba_sync_next(gba, GBA_SYNC_DMA, timing == GBA_DMA_HBLANK ? 0 : 1);
}
...
void gba_dma_notify_display_start(struct gba *gba, uint8_t vcount)
{
     ...
     bool video_oneshot = ch->video_oneshot;
     if (ch->enable && ch->timing == GBA_DMA_SPECIAL &&
         (!ch->pending || !video_oneshot))
     {
          ch->pending = true;
          /*
           * The single-shot "video transfer DMA" pulse measured by hw-test is
