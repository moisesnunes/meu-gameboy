                               ...
                               Sm83SemanticSignal signal, int bit)
{
     ...
     case SM83_SEM_PCL:
          if (bit > 7)
               return -1;
          return (gb->cpu.pc >> bit) & 1;
     case SM83_SEM_PCH:
          if (bit > 7)
               return -1;
          return (gb->cpu.pc >> (8 + bit)) & 1;
     case SM83_SEM_REG_A:
          if (bit > 7)
               return -1;
          return (gb->cpu.a >> bit) & 1;
     case SM83_SEM_REG_B:
          if (bit > 7)
               return -1;
          return (gb->cpu.b >> bit) & 1;
     case SM83_SEM_REG_C:
          if (bit > 7)
               return -1;
          return (gb->cpu.c >> bit) & 1;
     case SM83_SEM_REG_D:
          if (bit > 7)
               return -1;
          return (gb->cpu.d >> bit) & 1;
     case SM83_SEM_REG_E:
          if (bit > 7)
               return -1;
          return (gb->cpu.e >> bit) & 1;
     case SM83_SEM_REG_H:
          if (bit > 7)
               return -1;
          return (gb->cpu.h >> bit) & 1;
     case SM83_SEM_REG_L:
          if (bit > 7)
               return -1;
          return (gb->cpu.l >> bit) & 1;
     case SM83_SEM_SPL:
          if (bit > 7)
               return -1;
          return (gb->cpu.sp >> bit) & 1;
     case SM83_SEM_SPH:
          if (bit > 7)
               return -1;
          return (gb->cpu.sp >> (8 + bit)) & 1;
     case SM83_SEM_IR:
          if (bit > 7)
               return -1;
          return (gb->debug.cpu_viz.opcode >> bit) & 1;
     case SM83_SEM_DBUS:
          if (bit > 7)
               return -1;
          return (gb->debug.cpu_viz.data_bus >> bit) & 1;
     case SM83_SEM_FLAG_Z:
          if (bit != 0)
               return -1;
          return gb->cpu.f_z ? 1 : 0;
     case SM83_SEM_FLAG_N:
          if (bit != 0)
               return -1;
          return gb->cpu.f_n ? 1 : 0;
     case SM83_SEM_FLAG_H:
          if (bit != 0)
               return -1;
          return gb->cpu.f_h ? 1 : 0;
     case SM83_SEM_FLAG_C:
          if (bit != 0)
               return -1;
          return gb->cpu.f_c ? 1 : 0;
     case SM83_SEM_VCC:
          return 1;
     case SM83_SEM_GND:
          return 0;
     case SM83_SEM_IDU:
          return -1; /* IDU output not directly in gb->cpu */
     default:
          return -1;
     }
}
