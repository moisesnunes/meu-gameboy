...
void sm83_sim_init(Sm83NetlistSim *sim)
{
     ...
     {
          if (sim->vcc_net < 0)
               sim->vcc_net = SM83_VCC_NET_HEURISTIC;
          if (sim->gnd_net < 0)
               sim->gnd_net = SM83_GND_NET_HEURISTIC;
...
static void seed_net(Sm83NetlistSim *sim, const char *net_name, bool high)
{
     ...
     if (idx < 0)
          return;
     /* Rails are immutable */
     if (idx == sim->vcc_net || idx == sim->gnd_net)
          return;
                         ...
                         const Sm83CpuSnapshot *snap)
{
     ...
     case EVT_WRITEBACK:
          ...
          case VIZ_A:
               ...
               break;
          case VIZ_B:
               seed_bits8(sim, reg_b, ev_data8);
               break;
          case VIZ_C:
               seed_bits8(sim, reg_c, ev_data8);
               break;
          case VIZ_D:
               seed_bits8(sim, reg_d, ev_data8);
               break;
          case VIZ_E:
               seed_bits8(sim, reg_e, ev_data8);
               break;
          case VIZ_H:
               seed_bits8(sim, reg_h, ev_data8);
               break;
          case VIZ_L:
               seed_bits8(sim, reg_l, ev_data8);
               break;
          case VIZ_SP:
...
{
     /* CONFLICT is sticky */
     if (a == SM83_SIM_CONFLICT || b == SM83_SIM_CONFLICT)
          return SM83_SIM_CONFLICT;
     ...
     if (SM83_SIM_IS_DRIVEN(a))
          return a;
     if (SM83_SIM_IS_DRIVEN(b))
          return b;
     ...
     if (a == SM83_SIM_FLOAT || b == SM83_SIM_FLOAT)
...
{
     if (!sim->net_state || !sim->net_source)
          return 0;
     if (!sim->rails_found)
     {
          sim->iterations = 0;
     ...
     for (iter = 0; iter < max_iters; iter++)
     {
          ...
          for (int i = 0; i < SM83_TRANSISTOR_COUNT; i++)
          {
               ...
               if (gate < 0 || s1 < 0 || s2 < 0)
                    continue;
               if (gate >= SM83_NET_COUNT || s1 >= SM83_NET_COUNT || s2 >= SM83_NET_COUNT)
                    continue;

               Sm83SimState gs = (Sm83SimState)sim->net_state[gate];
               if (!transistor_conducts(tr->layer, gs))
                    continue;
     ...
     int conflicts = 0;
     for (int i = 0; i < SM83_NET_COUNT; i++)
          if (sim->net_state[i] == SM83_SIM_CONFLICT)
               conflicts++;
