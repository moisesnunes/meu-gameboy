...
static float conf_scale(HwMapConfidence c)
{
     switch (c)
     {
     case HW_CONF_CONFIRMED:
          return 1.0f;
     case HW_CONF_PROBABLE:
          return 0.80f;
     case HW_CONF_PROXY:
          return 0.50f;
     default:
...
/* LCD signal nets: LD0, LD1, CP, CPG, CPL, FR, S, ST */
static const int8_t LCD_NET_IDS[8] = {
    38,
    71,
    50,
    16,
...
/* Joypad matrix P10..P15 */
static const int8_t JOY_NET_IDS[6] = {
    17,
    51,
    36,
    45,
...
{
     if (addr <= 0x7FFF)
          return COMP_P1; /* ROM (cart)       */
     if (addr <= 0x9FFF)
          return COMP_U1; /* VRAM (internal)  */
     if (addr <= 0xBFFF)
          return COMP_P1; /* ext cart RAM     */
     if (addr <= 0xFDFF)
          return COMP_U2; /* WRAM             */
...
/* WRAM-internal address bus MA0..MA12.
 * The MA bus is driven by U1 whenever it accesses WRAM (0xC000-0xFDFF).
 * Only the low 13 bits of the CPU address are visible on MA (WRAM is 8 KiB). */
static void project_wram_addr(HwSchematicActivityState *st, uint16_t addr,
                              uint64_t seq, gb_hw_trace_event_type type)
{
     uint16_t ma = addr & 0x1FFF; /* 13-bit WRAM address */
     for (int i = 0; i < 13; i++)
     {
          int bit = (ma >> i) & 1;
          pulse_net(st, WRAM_ADDR_NET_IDS[i], 0.85f, seq, type, (int8_t)bit);
     }
}

/* WRAM-internal data bus MD0..MD7. */
static void project_wram_data(HwSchematicActivityState *st, uint8_t data,
                              uint64_t seq, gb_hw_trace_event_type type)
{
     for (int i = 0; i < 8; i++)
     {
          int bit = (data >> i) & 1;
          pulse_net(st, WRAM_DATA_NET_IDS[i], 0.85f, seq, type, (int8_t)bit);
     }
}

/* LCD pixel/control lines.  intensity varies by line: pixel data brighter. */
static void project_lcd(HwSchematicActivityState *st, float intensity,
                        uint64_t seq, gb_hw_trace_event_type type)
{
     for (int i = 0; i < 8; i++)
          pulse_net(st, LCD_NET_IDS[i], intensity, seq, type, -1);
}

/* Joypad: button_state byte encodes which buttons changed.
 * P10-P13 = directional (bits 0-3), P14-P15 = select lines (bits 4-5). */
static void project_joypad(HwSchematicActivityState *st, uint8_t button_state,
                           uint64_t seq, gb_hw_trace_event_type type)
{
     for (int i = 0; i < 6; i++)
     {
          int bit = (button_state >> i) & 1;
          /* active-low: button pressed = bit 0 = line pulled low */
          pulse_net(st, JOY_NET_IDS[i], 0.7f, seq, type, (int8_t)(bit ^ 1));
     }
}
...
void hw_project_event(HwSchematicActivityState *st, const gb_hw_trace_event *ev)
{
     ...
     case GB_HW_EVT_CPU_FETCH:
          /* PHI pulses on every M-cycle (each fetch = 1 M-cycle = 4 T-states) */
          pulse_net(st, NET_PHI, 0.6f, seq, type, 1);
          /* Fetch: address bus, data bus (opcode), /RD asserted */
          ...
          /* WRAM fetch (unlikely but possible via echo area) */
          if (addr >= 0xC000 && addr <= 0xFDFF)
          {
               project_wram_addr(st, addr, seq, type);
               project_wram_data(st, ev->opcode, seq, type);
               pulse_net(st, NET_nMCS, 1.0f, seq, type, 0);
               pulse_net(st, NET_nMRD, 1.0f, seq, type, 0);
          }
          break;

     case GB_HW_EVT_CPU_READ:
          pulse_net(st, NET_PHI, 0.5f, seq, type, 1);
          project_address(st, addr, seq, type);
          project_data(st, data, seq, type);
          pulse_net(st, NET_nRD, 1.0f, seq, type, 0);
          pulse_net(st, NET_nWR, 0.3f, seq, type, 1);
          /* Chip-select and WRAM internal buses */
          ...
          else if (addr >= 0xC000 && addr <= 0xFDFF)
          {
               ...
               pulse_net(st, NET_nMRD, 1.0f, seq, type, 0);
               project_wram_addr(st, addr, seq, type);
               project_wram_data(st, data, seq, type);
          }
          pulse_comp(st, COMP_U1, 1.0f, seq, type);
          pulse_comp(st, target_comp(addr), 0.9f, seq, type);
          break;
     ...
     case GB_HW_EVT_CPU_WRITE:
          ...
          {
               pulse_net(st, NET_nMCS, 1.0f, seq, type, 0);
               pulse_net(st, NET_nMWR, 1.0f, seq, type, 0);
               project_wram_addr(st, addr, seq, type);
               project_wram_data(st, data, seq, type);
          }
          pulse_comp(st, COMP_U1, 1.0f, seq, type);
     ...
     case GB_HW_EVT_IRQ_ACK:
          /* IRQ: no dedicated IRQ net in DMG schematic; just pulse CPU */
          pulse_comp(st, COMP_U1, 0.8f, seq, type);
          break;

     case GB_HW_EVT_DMA_READ:
          /* DMA bus-master read: external bus stays active at source address.
           * Cart and WRAM components light up depending on source region. */
          project_address(st, addr, seq, type);
          project_data(st, data, seq, type);
          pulse_net(st, NET_nRD, 0.8f, seq, type, 0);
          pulse_net(st, NET_PHI, 0.4f, seq, type, 1);
          if (addr >= 0xC000 && addr <= 0xFDFF)
          {
               pulse_net(st, NET_nMCS, 0.8f, seq, type, 0);
               pulse_net(st, NET_nMRD, 0.8f, seq, type, 0);
               project_wram_addr(st, addr, seq, type);
               project_wram_data(st, data, seq, type);
          }
          else if (addr <= 0xBFFF)
     ...
     case GB_HW_EVT_DMA_WRITE:
          /* DMA writes OAM (internal to U1 — external bus is blocked) */
          project_data(st, data, seq, type);
          pulse_net(st, NET_nWR, 0.8f, seq, type, 0);
          pulse_net(st, NET_PHI, 0.4f, seq, type, 1);
          pulse_comp(st, COMP_U1, 0.7f, seq, type);
          /* External bus is idle during OAM DMA block — deassert cart/WRAM CS */
          pulse_net(st, NET_nCS, 0.1f, seq, type, 1);
          pulse_net(st, NET_nMCS, 0.1f, seq, type, 1);
          break;

     case GB_HW_EVT_PPU_MODE:
          /* PPU mode transition: data=new mode (0-3), extra=LY.
           * Mode 3 (drawing): pixel data + CP active; mode 2: OAM scan. */
          {
               float lcd_int = (ev->data == 3) ? 0.9f : (ev->data == 2) ? 0.5f
                                                    : (ev->data == 1)   ? 0.7f
                                                                        : 0.3f;
               project_lcd(st, lcd_int, seq, type);
     ...
     case GB_HW_EVT_PPU_VBLANK:
          /* VBlank: FR signal asserts (frame reset), all LCD lines active */
          project_lcd(st, 1.0f, seq, type);
          pulse_comp(st, 11, 1.0f, seq, type); /* J2 = LCD */
          pulse_comp(st, COMP_U1, 0.8f, seq, type);
          break;

     case GB_HW_EVT_PPU_HBLANK:
          /* HBlank: ST (start pulse) may assert; pixel lines go quiet */
          project_lcd(st, 0.35f, seq, type);
          pulse_comp(st, 11, 0.4f, seq, type);
          pulse_comp(st, COMP_U1, 0.2f, seq, type);
          break;

     case GB_HW_EVT_OAM_DMA:
          /* OAM DMA byte: internal write to OAM; external bus is blocked.
           * addr=OAM destination (FE00-FE9F), data=byte copied. */
          project_data(st, ev->data, seq, type);
          pulse_net(st, NET_nWR, 0.8f, seq, type, 0);
          pulse_net(st, NET_PHI, 0.4f, seq, type, 1);
          pulse_comp(st, COMP_U1, 0.7f, seq, type);
          /* External CS lines are deasserted (bus blocked) */
          pulse_net(st, NET_nCS, 0.1f, seq, type, 1);
          pulse_net(st, NET_nMCS, 0.1f, seq, type, 1);
          break;
     ...
     case GB_HW_EVT_APU_WRITE:
          /* APU register write: addr=register (0xFF10-3F), data=value */
     ...
     case GB_HW_EVT_JOYPAD:
          /* Joypad: data=button state byte (active-low per DMG schematic).
           * Project onto real P10-P15 matrix lines. */
          project_joypad(st, ev->data, seq, type);
          pulse_comp(st, COMP_U1, 0.5f, seq, type);
          break;

     case GB_HW_EVT_SERIAL_START:
          /* Serial transfer begin: SCK/SIN/SOUT active on link port.
           * extra=SC byte; serial is off the main bus. */
     ...
     case GB_HW_EVT_MBC_SWITCH:
          /* MBC bank switch: write to cart address space triggers bank register.
...
int hw_activity_consume_trace(HwSchematicActivityState *st,
                              ...
                              uint64_t *last_seq)
{
     if (!st || !trace || !last_seq)
          return 0;
     if (!trace->enabled || trace->count == 0)
          return 0;
...
static void audit_push(HwAuditResult *out, HwAuditCategory cat,
                       ...
                       const char *detail)
{
     if (out->count >= HW_AUDIT_MAX_FINDINGS)
          return;
