...
struct gb_spu_duration
{
     ...
     bool enable;
     /* Remaining length ticks. Clocked by the 256 Hz frame sequencer steps. */
     uint16_t counter;
};
...
struct gb_spu_sweep
{
     ...
     uint8_t time;
     /* Frame sequencer ticks to the next sweep step */
     uint8_t counter;
     /* Frequency copied on trigger and modified by sweep ticks */
     uint16_t shadow_offset;
     /* True if sweep is active after trigger */
     bool enable;
     /* True after a subtraction sweep calculation */
...
struct gb_spu_envelope
{
     ...
     /* Step counter */
     uint8_t counter;
     /* True while envelope ticks can still change volume */
     bool running;
};
...
struct gb_spu_nr3
{
     ...
     uint8_t index;
     /* DMG: cycles remaining in the wave RAM CPU-access window. The window
      * opens (set to 2) each time the wave unit reads a new byte and is
      * decremented every SPU cycle. When it reaches 0 the window is closed:
      * reads return 0xFF and writes are ignored. Always 0 on CGB. */
...
struct gb_spu
{
     ...
     /* 512 Hz APU frame sequencer state */
     ...
     /* Frontend-only channel mute switches. They affect mixing but do not
      * change emulated APU register state. */
