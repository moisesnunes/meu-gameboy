...
static void draw_lcd_overlay(SDL_Renderer *r, SDL_FRect lcd, int px_w, int px_h)
{
     ...
     rv[1].position.x = lcd.x + lcd.w;
     rv[1].position.y = lcd.y;
     rv[2].position.x = lcd.x;
     rv[2].position.y = lcd.y + lcd.h;
...
static void handle_key(struct gb *gb, SDL_Keycode key, bool pressed)
{
     ...
     case SDLK_ESCAPE:
          ...
          break;
     case SDLK_RETURN:
          set_button(gb, VEC_BTN_START, GB_INPUT_START, pressed);
          break;
     case SDLK_RSHIFT:
          set_button(gb, VEC_BTN_SELECT, GB_INPUT_SELECT, pressed);
          break;
     case SDLK_LCTRL:
          set_button(gb, VEC_BTN_A, GB_INPUT_A, pressed);
          break;
     case SDLK_LSHIFT:
          set_button(gb, VEC_BTN_B, GB_INPUT_B, pressed);
          break;
     case SDLK_UP:
          set_button(gb, VEC_BTN_UP, GB_INPUT_UP, pressed);
          break;
     case SDLK_DOWN:
          set_button(gb, VEC_BTN_DOWN, GB_INPUT_DOWN, pressed);
          break;
     case SDLK_LEFT:
          set_button(gb, VEC_BTN_LEFT, GB_INPUT_LEFT, pressed);
          break;
     case SDLK_RIGHT:
          set_button(gb, VEC_BTN_RIGHT, GB_INPUT_RIGHT, pressed);
          break;
     case SDLK_TAB:
...
int main(int argc, char **argv)
{
     ...
     while (!gb->quit)
     {
          ...
          int32_t cycles = (int32_t)((float)(elapsed * (uint64_t)GB_CPU_FREQ_HZ / 1000000000ULL) * speed);
          if (cycles > 0)
