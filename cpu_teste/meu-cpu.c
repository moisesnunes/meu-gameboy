...
void gb_cpu_reset(struct gb *gb)
{
     ...
     /* XXX Por enquanto não emulamos a BOOTROM, então iniciamos a execução logo após ela */
     cpu->pc = 0x100;

     if (gb->gbc)
     {
          /* No modo GBC, a boot ROM define A como 0x11 antes de iniciar o jogo.
           * O jogo pode usar isso para detectar se esta rodando em DMG ou GBC.
           */
          cpu->a = 0x11;
     }
}
...
static void gb_i_undefined(struct gb *gb)
{
     ...
     // Opcode não definido. Aparentemente trava a CPU no hardware real
 ...
 *
 * Deve limpar o registrador IME (Interrupt Master Enable), impedindo
 ...
 * TODO: implementar a limpeza do flag IME.
 */
...
static void gb_i_stop(struct gb *gb)
{
     if (gb->speed_switch_pending)
     {
          ...
          /* XXX: isto deveria levar cerca de 16ms ao entrar em double-speed e
           * cerca de 32ms ao voltar ao modo normal. */
...
static void gb_i_halt(struct gb *gb)
{
     ...
     {
          /* HALT bug: IME=0 com interrupcao pendente. A CPU nao para; em vez
           * disso, a proxima busca de opcode nao avanca o PC. */
 ...
 *
 * Instruções implementadas até o momento:
 *   0x00 - NOP          (sem operação)
 *   0x31 - LD SP, nn    (inicializa o Stack Pointer)
 *   0x3E - LD A, n      (carrega imediato em A)
 *   0x97 - SUB A        (zera o acumulador)
 *   0xC3 - JP nn        (salto incondicional)
 *   0xC9 - RET          (retorno de sub-rotina)
 *   0xCD - CALL nn      (chamada de sub-rotina)
 *   0xE0 - LDH (n), A   (escreve A em registrador de I/O)
 *   0xE8 - ADD SP, e    (ajusta SP com offset com sinal) [BUG: SP não é atualizado]
 *   0xEA - LD (nn), A   (armazena A em endereço absoluto)
 *   0xF3 - DI           (desabilita interrupções) [TODO: IME não implementado]
 */
...
static gb_instruction_f gb_instructions[0x100] = {
    ...
    gb_i_ld_sp_i16, /* 0x31: LD SP, nn  ← implementado */
    ...
    gb_i_ld_a_i8,   /* 0x3E: LD A, n  ← implementado */
    gb_i_ccf,       /* 0x3F: CCF */
    ...
    gb_i_sub_a_a,   /* 0x97: SUB A  ← implementado */
    ...
    gb_i_jp_i16,      /* 0xC3: JP nn  ← implementado */
    ...
    gb_i_ldh_mi8_a,  /* 0xE0: LDH (n), A  ← implementado */
    ...
    gb_i_add_sp_si8, /* 0xE8: ADD SP, e  ← implementado */
    gb_i_jp_hl,      /* 0xE9: JP (HL) — salto indireto para endereço em HL */
    gb_i_ld_mi16_a,  /* 0xEA: LD (nn), A  ← implementado */
    ...
    gb_i_di,           /* 0xF3: DI  ← implementado (TODO: IME) */
 ...
 *
 * Instruções implementadas até o momento:
 *   0x30–0x37 - SWAP r / SWAP (HL)  (troca nibbles)
 * Todas as demais entradas apontam para gb_i_unimplemented_cb.
 */
static gb_instruction_f gb_instructions_cb[0x100] = {
    ...
    // 0x30 - SWAP r : troca nibble alto e nibble baixo ← implementado
