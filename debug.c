...
void gb_debug_before_instr(struct gb *gb)
{
     ...
     /* ── CPU viz: capture per-instruction state ── */
     {
          ...
          {
               uint8_t flags_now = (uint8_t)((gb->cpu.f_z ? 8u : 0u) |
                                             (gb->cpu.f_n ? 4u : 0u) |
                                             (gb->cpu.f_h ? 2u : 0u) |
                                             (gb->cpu.f_c ? 1u : 0u));
          ...
          cv->alu_a = gb->cpu.a;
          cv->flags_before = (uint8_t)((gb->cpu.f_z ? 8u : 0u) |
                                       (gb->cpu.f_n ? 4u : 0u) |
                                       (gb->cpu.f_h ? 2u : 0u) |
                                       (gb->cpu.f_c ? 1u : 0u));
          ...
          case GB_VIZ_REG_L:
               ...
               break;
          default:
               cv->alu_b = cv->data_bus;
               break; /* IMM/MEM: use last bus value */
          }
     }
...
void gb_debug_add_breakpoint(struct gb *gb, uint16_t addr)
{
     ...
     if (dbg->n_breakpoints >= GB_DEBUG_MAX_BREAKPOINTS)
          return;
...
void gb_debug_add_watchpoint(struct gb *gb, uint16_t addr, gb_watchpoint_type type)
{
     ...
     if (dbg->n_watchpoints >= GB_DEBUG_MAX_WATCHPOINTS)
          return;
...
void gb_debug_check_watchpoint(struct gb *gb, uint16_t addr, gb_watchpoint_type type)
{
     ...
     if (!dbg->enabled || dbg->state != GB_DEBUG_RUNNING)
          return;
