...
static void test_layer_ranges(void)
{
     ...
     int arc_sum = (SM83_ARC_METAL1_END - SM83_ARC_METAL1_START) + (SM83_ARC_POLY_END - SM83_ARC_POLY_START) + (SM83_ARC_NACTIVE_END - SM83_ARC_NACTIVE_START) + (SM83_ARC_PACTIVE_END - SM83_ARC_PACTIVE_START);
...
static void test_transistor_nets(void)
{
     ...
     for (int i = 0; i < SM83_TRANSISTOR_COUNT; i++)
     {
          ...
          {
               with_gate++;
               if (t->gate_net >= SM83_NET_COUNT)
                    bad_gate++;
          }
          if (t->s1_net >= SM83_NET_COUNT || t->s2_net >= SM83_NET_COUNT)
               bad_s++;
          if (t->s1_net < -1 || t->s2_net < -1)
               bad_s++;
     }
...
static void test_rail_propagation(void)
{
     ...
     for (int i = 0; i < SM83_NET_COUNT; i++)
     {
          ...
          case SM83_SIM_HIGH:
          case SM83_SIM_LOW:
          case SM83_SIM_HIGH_WEAK:
          case SM83_SIM_LOW_WEAK:
               resolved++;
               break;
          case SM83_SIM_FLOAT:
               floating++;
               break;
          case SM83_SIM_CONFLICT:
               conflicted++;
               break;
          default:
               unknown++;
...
static void test_semantic_map(void)
{
     ...
     for (int i = 0; i < total; i++)
     {
          ...
          {
               if (sm83_semantic_map[j].net_id != nid)
                    continue;
